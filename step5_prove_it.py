...
Two BEFORE baselines, reported honestly:
  naive_full_content  -- what a tool naively returning full wikitext per
                          revision would send (projected from a sampled
                          avg tokens/revision, since fetching full content
                          for thousands of revisions is impractical)
  naive_metadata_only -- the fairer baseline: what a tool returning just
                          revision metadata (no content) would send

AFTER is the real investigate_conflict() brief for the query.
